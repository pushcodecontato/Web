...
class LocacaoDAO{
    ...
    public function selectAll($status,$id_cliente){
       ...
       if($status == "andamento"){
            ...
            $sql = "SELECT tbl_locacao.*,tbl_solicitacao_anuncio.id_solicitacao_anuncio FROM tbl_anuncio inner join tbl_locacao on (tbl_locacao.id_anuncio = tbl_anuncio.id_anuncio AND tbl_locacao.data_hora_inicial is not null AND tbl_locacao.data_hora_final is null) inner join tbl_solicitacao_anuncio on  tbl_locacao.id_locacao = tbl_solicitacao_anuncio.id_locacao where tbl_anuncio.id_cliente_locador=$id_cliente";
